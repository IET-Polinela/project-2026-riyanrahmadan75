...
def search_reports_api(request):
    ...
    if not user.is_superuser:
        reports = reports.filter(Q(reporter=user) | ~Q(status='DRAFT'))

    reports = reports.values('id','title','location','status')[:10]
...
def report_detail_api(request, pk):
    try:
        ...
        if report.status == 'DRAFT' and report.reporter != user and not user.is_superuser:
            return JsonResponse({'error': 'Laporan tidak ditemukan.'}, status=404)

        if user.is_superuser or report.reporter == user:
